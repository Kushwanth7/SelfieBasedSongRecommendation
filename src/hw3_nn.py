...
class myMLP(object):
    ...
    def __init__(self, rng, input, n_in, n_hidden, n_out, n_hiddenLayers):
        ...
        #P of y given x
        self.pOfYGivenX = self.logRegressionLayer.pOfYGivenX
...
def generateGaussian(shape=(5,5),sigma=0.5):
    ...
    if sumh != 0:
        h /= sumh
    return h
def generateGaussianFilter():
    filter1 = generateGaussian(shape=(5,5),sigma=0.4)
    filter2 = generateGaussian(shape=(5,5),sigma=0.5)
    filter3 = generateGaussian(shape=(5,5),sigma=0.6)
    filter1 = filter1.reshape((1,5,5))
    filter2 = filter2.reshape((1,5,5))
    filter3 = filter3.reshape((1,5,5))
    filter1 = filter1.repeat(3, axis=0)
    filter2 = filter2.repeat(3, axis=0)
    filter3 = filter3.repeat(3, axis=0)
    a = numpy.empty((3,3,5,5))
    a[0] = filter1
    a[1] = filter2
    a[2] = filter3
    return a
...
class LeNetConvPoolLayer(object):
    ...
    def __init__(self, rng, input, filter_shape, image_shape, poolsize=(2, 2),
        pool_ignore_border=True,layerNumber = 1):
        """
